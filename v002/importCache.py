...
from ptCacheTools import importUI as ui
reload(ui)
...
class MyForm(QtGui.QMainWindow) : 
	...
	def initialConnection(self) : 
		...
		self.ui.buildSet_pushButton.clicked.connect(self.importSet)
		self.ui.buildNonCache_pushButton.clicked.connect(self.importNonCacheAsset)
	...
	def menuCommand(self, command, item) : 
		assetName = item
		data = self.cacheCommand.readAssetReferenceFile()
		path = ''
		basePath = ''
	...
	def removeReferenceAction(self) : 
		assets = []
	...
	def listNonCache(self) : 
		info = self.cacheCommand.info()
		nonCachePath = info['nonCachePath']
		print nonCachePath

		if os.path.exists(nonCachePath) : 
			baseName = os.path.basename(nonCachePath)
			self.ui.nonCache_label.setText(baseName)

			self.ui.buildNonCache_pushButton.setEnabled(True)

		else : 
			self.ui.buildNonCache_pushButton.setEnabled(False)
			self.ui.nonCache_label.setText('N/A')
	...
	def importSet(self) : 
		...
		# mc.file(setPath, r = True, type = 'mayaAscii', ignoreVersion = True, gl = True, loadReferenceDepth = 'all', namespace = 'set', options = 'v=0')

		self.setStatusBuildSet_pushButton()
