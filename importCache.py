...
from ptCacheTools import importUI4 as ui
reload(ui)
...
class MyForm(QtGui.QMainWindow) : 
	...
	def showMenu(self, pos) : 
		...
		if item : 
			...
			subMenuItem = subMenu.addAction('Reload')
	...
	def menuCommand(self, command, item, category) : 
		...
		if command == 'Reload' : 
			self.switchReference(True, True)
	...
	def switchReference(self, applyAnimation = True, reload = False) : 
		if reload : 
			self.assetMap = {'Render': 'Render', 'Cache': 'Cache'}
	...
	def listExportAsset(self) : 
		info = self.cacheCommand.info()
		exportAsset = info['exportAsset']
		
		if os.path.exists(exportAsset) : 
			self.ui.importAsset_pushButton.setText('Import Crowd')

		else : 
			self.ui.importAsset_pushButton.setText('No Crowd')
