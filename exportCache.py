...
from ptCacheTools import exportUI4 as ui
reload(ui)

from ptCacheTools import openNoteCore
reload(openNoteCore)

from ptCacheTools import cacheTools
reload(cacheTools)

# turn on plugin
cacheTools.loadPlugin()
...
class MyForm(QtGui.QMainWindow) : 
	...
	def initConnection(self) : 
		...
		self.ui.exportSet_pushButton.clicked.connect(self.exportSetNonCache)
		...
		self.ui.exportAsset_pushButton.clicked.connect(self.exportAsset)
	...
	def exportCacheAction(self) : 
		...
		if not self.ui.manual_checkBox.isChecked() : 
			if self.ui.oneTime_checkBox.isChecked() : 
				...
				self.exportAnimCurve(False, True)
				self.exportLog()
	...
	def exportSelectedCache(self) : 
		if self.ui.oneTime_checkBox.isChecked() : 
			self.cacheSelected()
			self.exportAnimCurve(False, False)
			self.exportLog()
	...
	def listSetUI(self) : 
		...
		nonCache = self.cacheCommand.listNonCache()
		for each in nonCache : 
			self.ui.set_listWidget.addItem(each)


	def listExportUI(self) : 
		if mc.objExists(self.exportGrp) : 
			self.ui.exportAsset_pushButton.setEnabled(True)
			self.ui.exportAsset_pushButton.setText('Export Crowd Group')

		else : 
			self.ui.exportAsset_pushButton.setEnabled(False)
	...
	# 	items = self.getAllWidgetItem('set_listWidget')
	...
	def exportAsset(self) : 
		targetGrp = self.exportGrp

		if mc.objExists(targetGrp) : 
			result = self.cacheCommand.exportAssetCmd(targetGrp)

			if result : 
				self.setIconExportAsset_pushButton('OK')

		else : 
			self.setIconExportAsset_pushButton('No')


	def exportSetNonCache(self) : 
		items = self.getAllWidgetItem('set_listWidget')
		...
		for eachItem in items : 
			if mc.objExists('%s:Rig_Grp' % eachItem) : 
				existItem+=1

		if allItems == existItem : 
			result = self.cacheCommand.exportSetNonCacheCmd(items)
			self.cacheCommand.exportReferenceDB(items)
	...
	def exportAnimCurve(self, dialog = True, all = True) : 
		...
		if all : 
			...
			items = [item.text() for item in self.ui.asset_listWidget.selectedItems()]
			print items
		...
		for each in items : 
			...
			if mc.objExists(geoGrp) : 
				...
				try : 
					self.cacheCommand.exportAnim(each, exportPath)

				except : 
					print 'Cannot export animCurve for %s' % each
				# mc.select(cl = True)

		print 'Export Anim finished'
	...
	def setIconExportAsset_pushButton(self, status = 'OK') : 
		if os.path.exists(self.okIcon) : 
			icon = QtGui.QIcon()
			icon.addPixmap(QtGui.QPixmap(self.okIcon),QtGui.QIcon.Normal,QtGui.QIcon.Off)

			if status == 'OK' : 
				self.ui.exportAsset_pushButton.setIcon(icon)
				self.ui.exportAsset_pushButton.setText('Export Crowd Group')

			else : 
				self.ui.exportAsset_pushButton.setText('No export_grp')
...
class cmd() : 
	def __init__(self) : 
		...
		self.exportAssetPath = str()

		self.pipelineStatus = self.setPathInfo()

		self.exception = ['export_grp']
	...
	def setPathInfo(self) : 
		try : 
			...
			nonCachePath = mayaTools.fixPath(os.path.join(currentLevel, 'cache', 'nonCacheAsset'), '/')
	...
	def listAsset(self) : 
		...
		for each in assets : 
			if mc.listRelatives(each, p = True) : 
				upGroup = mc.listRelatives(each, p = True)[0]

			if not upGroup in exception : 
	...
	def listNonCache(self) : 
		allAssets = []
		caches = self.listAsset()
		nonCache = []
		assets = mc.ls('*:Rig_Grp')
		exception = self.exception
		upGroup = str()
		...
		for each in assets : 
			...
			if not upGroup in exception : 
				namespace = mayaTools.getNamespace(each)
				if not namespace in allAssets : 
					allAssets.append(namespace)

		nonCache = [asset for asset in allAssets if not asset in caches]
	...
	def exportSetAnimCurve(self, namespace, bake = True) : 
		info = self.info()

		animPath = '%s/animCurve' % info['nonCachePath']
		...
		infoPath = '%s/%s' % (animPath, '%s.txt' % namespace)
		fileUtils.writeFile(infoPath, str(ctrls))

		tempFile = '%s/%s' % (mc.internalVar(utd = True), '%s.anim' % namespace)
		animFile = '%s/%s' % (animPath, '%s.anim' % namespace)

		mc.select(ctrls)
		# mc.select('%s:Rig_Grp' % namespace)
		...
		# mm.eval('file -force -options "precision=8;intValue=17;nodeNames=1;verboseUnits=0;whichRange=1;range=1:24;options=keys;hierarchy=below;controlPoints=0;shapes=0;helpPictures=0;useChannelBox=0;copyKeyCmd=-animation objects -option keys -hierarchy below -controlPoints 0 -shape 0 " -typ "animExport" -pr -es "%s";' % animFile)
	...
	def exportAssetCmd(self, obj) : 

		# export path
		info = self.info()
		filePath = info['exportAssetPath']

		path = filePath.replace('/%s' % os.path.basename(filePath), '')
		...
		# export
		mc.select(obj)
		mc.file(filePath, force = True, options = 'v=0', type = 'mayaAscii', pr = True, es = True)
		mc.select(cl = True)

		return True


	def exportSetNonCacheCmd(self, objs) : 
		info = self.info()
		path = '%s/mayaFiles' % info['nonCachePath']
		success = 0
		...
		for eachAsset in allAssets : 
			asset = '%s:Rig_Grp' % eachAsset
			exportPath = '%s/%s' % (path, eachAsset)

			if not os.path.exists(path) : 
				os.makedirs(path)

			mc.select(asset)
			mc.file(exportPath, force = True, options = 'v=0', type = 'mayaAscii', pr = True, es = True)

			self.exportSetAnimCurve(eachAsset)

			success += 1

		mayaTools.isolateObj(False)

		if success == len(allAssets) : 
			return True
			...
			for eachKey in removeKeywords : 
				if eachKey in ns : 
					...
					try : 
						...
						mm.eval('warning "Failed to rename namespace %s\\n"' % ns)
	...
	def exportAnim(self, namespace, exportFile) : 
		set1 = mc.ls('%s:*_ctrl' % namespace)
		set2 = mc.ls('%s:*:*_ctrl' % namespace)
		set3 = mc.ls('%s:*:*:*_ctrl' % namespace)
		set4 = mc.ls('%s:*_Ctrl' % namespace)
		...
		# mm.eval('file -force -options "precision=8;intValue=17;nodeNames=1;verboseUnits=0;whichRange=1;range=1:24;options=keys;hierarchy=below;controlPoints=0;shapes=0;helpPictures=0;useChannelBox=0;copyKeyCmd=-animation objects -option keys -hierarchy below -controlPoints 0 -shape 0 " -typ "animExport" -pr -es "%s";' % tempFile)
		mm.eval('file -force -options "precision=8;intValue=17;nodeNames=1;verboseUnits=0;whichRange=1;range=1:24;options=keys;hierarchy=none;controlPoints=0;shapes=0;helpPictures=0;useChannelBox=0;copyKeyCmd=-animation objects -option keys -hierarchy none -controlPoints 0 -shape 0 " -typ "animExport" -pr -es "%s";' % tempFile)
		...
		exportFilePath = '%s/%s.txt' % (exportFile.replace('/%s' % os.path.basename(exportFile), ''), namespace)
